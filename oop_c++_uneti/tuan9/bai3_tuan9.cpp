...
void tieude()
{
	...
	cout << left << setw(10) << "Ma hd"
		 ...
		 << left << setw(20) << "K.luong/Gio thue"
	 	 << left << setw(10) << "Don gia"
	 	 << left << setw(30) << "Tien giat la/Tien thue xe"
		 << endl;
}
	...
	do
	{
		...
		switch (luaChon2)
		{
			case 1: 
			{
				...
				cout << "\t______________________________DANH SACH THONG TIN CAC LOAI DICH VU_____________________________" << endl;
				tieude();
				for (int i = 0; i < n; i++)
				{
					d[i]->xuat();
			...
			case 3:
			{
				cout << "\t____________________DANH SACH THONG TIN DICH VU CO TONG TIEN HON 10 TRIEU______________________" << endl;
				tieude();
				for (int i = 0; i < n; i++)
				{
					if (d[i]->phanLoai() == 1)
